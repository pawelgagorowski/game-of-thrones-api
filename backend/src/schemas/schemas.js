...
const createMembersSchema = Joi.object().keys({
  name: Joi.string().trim().required(),
  surname: Joi.string().trim().required()
})
.or('name', 'surname')

const createClanSchema = Joi.object().keys({
  clan: Joi.string().trim().required()
})

// export the schemas
module.exports = {
    '/api/v1/clans': createClanSchema,
    'api/v1/clans/stark/members': createMembersSchema
};
